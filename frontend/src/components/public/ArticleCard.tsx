    ...
    <Link to={`/blog/${article.slug}`} className="block group">
      <div className="rounded-xl bg-white border border-gray-100 hover:border-primary/20 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 overflow-hidden h-full flex flex-col">
            ...
            <ImageWithLazyLoad
              src={
                article.coverImage.startsWith("http")
                  ? article.coverImage
                  : `${import.meta.env.VITE_API_URL?.replace("/api", "") || "http://localhost:3000"}${article.coverImage}`
              }
              alt={article.title}
        ...
        <div className="p-5 flex-1 flex flex-col">
          ...
          {/* Excerpt */}
          {article.excerpt && (
            <p className="text-sm text-black line-clamp-2 mb-3 flex-1">
              {article.excerpt}
            </p>
          )}
          ...
          {article.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3">
              {article.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-0.5 rounded-full bg-gray-50 text-primary text-xs"
                >
                  #{tag}
                </span>
              ))}
