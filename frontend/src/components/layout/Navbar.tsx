...
export default function Navbar() {
  ...
  return (
    <nav
      className="sticky top-0 z-50 bg-gradient-to-r from-white via-white/30 to-white backdrop-blur-2xl border-b border-white/20 shadow-xs"
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <svg
                      className={`w-4 h-4 text-black transition-transform ${profileOpen ? "rotate-180" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                      aria-hidden="true"
          ...
          <div className="px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <Link
                ...
                className={`block px-4 py-2.5 rounded-xl text-black text-sm font-medium transition-colors ${
                  isActive(link.path)
                    ? "bg-primary/10 text-primary"
            ...
            {user ? (
              <div className="space-y-1">
                ...
                <Link
                  to="/dashboard"
                  onClick={() => setMobileOpen(false)}
                  className="block px-4 py-2.5 rounded-xl text-sm text-gray-700 hover:bg-gray-50"
                >
                  Dashboard
                </Link>
                <Link
                  to="/notifications"
                  onClick={() => setMobileOpen(false)}
                  className="flex items-center justify-between px-4 py-2.5 rounded-xl text-sm text-gray-700 hover:bg-gray-50"
                >
              ...
              <div className="flex gap-2">
                <Link
                  to="/login"
                  onClick={() => setMobileOpen(false)}
                  className="flex-1 text-center px-4 py-2.5 text-sm font-medium text-gray-700 border border-gray-200 rounded-xl hover:bg-gray-50"
                >
                  Masuk
                </Link>
                <Link
                  to="/register"
                  onClick={() => setMobileOpen(false)}
                  className="flex-1 text-center px-4 py-2.5 text-sm font-semibold text-white bg-linear-to-r from-primary to-secondary rounded-xl"
                >
                  Daftar
                </Link>
              </div>
