...
export default function ForumDetail() {
  ...
  const fetchTopic = useCallback(async () => {
    ...
    setError("");
    try {
      // Fetch main topic data first as it's critical
      ...
      // Fetch related topics separately to avoid blocking the main view if it fails
      try {
        const { data: relatedData } = await axios.get(`${API_URL}/forum/${id}/related`);
        setRelatedTopics(relatedData);
      } catch (relErr) {
        console.error("Gagal memuat topik terkait:", relErr);
        setRelatedTopics([]);
      }
    } catch (err) {
      console.error("Gagal memuat topik:", err);
      setError("Topik tidak ditemukan atau terjadi kesalahan server.");
    } finally {
      setLoading(false);
    }
  ...
  const executeDeleteComment = async () => {
    ...
    try {
      await axios.delete(`${API_URL}/forum/comments/${commentToDelete}`, {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-xl shadow-xl border border-gray-100 max-w-sm w-full text-center">
          ...
          <Link to="/forum" className="text-primary font-bold hover:underline">
            Kembali ke Forum
          </Link>
        </div>
      </div>
  ...
  const categorySlug = (topic.category?.slug ||
    "lainnya") as keyof typeof categoryColors;
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
                ...
                <div className="flex gap-6 text-xs text-gray-400">
                  <div className="flex flex-row items-center justify-between">
                    <div className="flex items-center gap-1 group/like">
                      ...
                      <span
                        className={`text-sm font-bold min-w-4 text-center ${user && topicLikes.includes(user._id) ? "text-primary" : "text-gray-700"}`}
                      >
                        {topicLikes.length}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1 group/dislike">
                      ...
                      <span
                        className={`text-sm font-bold min-w-4 text-center ${user && topicDislikes.includes(user._id) ? "text-red-500" : "text-gray-700"}`}
                      >
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {relatedTopics.map((t) => (
                  <Link
                    key={t._id}
                    to={`/forum/${t.slug || t._id}`}
                    className="p-4 bg-white rounded-xl border border-gray-100 hover:border-primary/30 hover:shadow-md transition-all group"
                  >
                    <p className="text-xs text-primary font-bold mb-1 opacity-60 group-hover:opacity-100 transition-opacity">
                      Related Thread
                    </p>
      ...
      <ConfirmModal
        ...
        title="Hapus Topik"
        message="Hapus topik ini beserta semua komentarnya? Tindakan ini tidak bisa dibatalkan."
        confirmLabel="Ya, Hapus Topik"
        cancelLabel="Batal"
        variant="danger"
        onConfirm={executeDeleteTopic}
        onCancel={() => setIsTopicDeleteModalOpen(false)}
      />
      ...
      <ConfirmModal
        ...
        message="Hapus komentar ini? Tindakan ini tidak bisa dibatalkan."
